...
class FeatureGraphsWidget(QWidget):
    ...
    update_user_data = pyqtSignal(
        np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray)
    update_static_data = pyqtSignal(
    ...
    def __init__(self):
        ...
        self.filtered_user_trajectory = pg.PlotWidget(
            title="filtered_user_trajectory")
        self.discrete_user_trajectory = pg.PlotWidget(
            title="discrete_user_trajectory")
        self.discrete_reference_trajectory = pg.PlotWidget(
            title="discrete_reference_trajectory")
        self.filtered_reference_trajectory = pg.PlotWidget(
            title="filtered_reference_trajectory")
        ...
        self.filtered_user_trajectory.addItem(
            self.filtered_user_trajectory_curve)
        self.reference_trajectory.addItem(self.reference_trajectory_curve)
        self.discrete_user_trajectory.addItem(
            self.discrete_user_trajectory_curve)
        self.filtered_reference_trajectory.addItem(
            self.filtered_reference_trajectory_curve)
        self.discrete_reference_trajectory.addItem(
            self.discrete_reference_trajectory_curve)
        self.errors.addItem(self.errors_curve)
        self.progress_vector.addItem(self.progress_vector_curve)

        self.feature_index_pointer = CurveArrow(
            self.discrete_reference_trajectory_curve, 0)
        self.feature_index_pointer.setStyle(
            angle=270, headWidth=5, pen=GYMY_ORANGE, brush=GYMY_ORANGE)
    ...
    def _update_user_data(self, user_trajectory, filtered_user_trajectory, discrete_user_trajectory, errors, progress_vector, prediction):
        ...
        self._filtered_user_trajectory_x = np.array(
            range(len(filtered_user_trajectory)))
        self._filtered_user_trajectory_y = filtered_user_trajectory

        self._discrete_user_trajectory_x = np.array(
            range(len(discrete_user_trajectory)))
    ...
    def update_plots(self):
        ...
        try:
            self.user_trajectory_curve.setData(
                self._user_trajectory_x, self._user_trajectory_y)
            self.discrete_user_trajectory_curve.setData(
                self._discrete_user_trajectory_x, self._discrete_user_trajectory_y)
            self.filtered_user_trajectory_curve.setData(
                self._filtered_user_trajectory_x, self._filtered_user_trajectory_y)
            self.feature_index_pointer.setIndex(self._prediction)
            self.errors_curve.setData(self._errors_x, self._errors_y)
            # TODO: draw vector here? https://stackoverflow.com/questions/44246283/how-to-add-a-arrow-head-to-my-line-in-pyqt4
            self.progress_vector_curve.setData(
                self._progress_vector_x, self._progress_vector_y, pen=GYMY_GREEN)
        except IndexError as e:  # This occurs if our progress points to an index higher than our trajectory length
            logy.warn_throttle(e)
    ...
    def _update_static_data(self, reference_trajectory, filtered_reference_trajectory, discrete_reference_trajectory, feature_type, feature_spec):
        ...
        self.reference_trajectory_curve.setData(
            reference_trajectory_x, reference_trajectory_y)

        filtered_reference_trajectory_x = np.array(
            range(len(filtered_reference_trajectory)))
        filtered_reference_trajectory_y = np.array(
            filtered_reference_trajectory)
        self.filtered_reference_trajectory_curve.setData(
            filtered_reference_trajectory_x, filtered_reference_trajectory_y)

        discrete_reference_trajectory_x = np.array(
            range(len(discrete_reference_trajectory)))
        discrete_reference_trajectory_y = discrete_reference_trajectory
        self.discrete_reference_trajectory_curve.setData(
            discrete_reference_trajectory_x, discrete_reference_trajectory_y)
...
class MotionAnaysisGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.overall_progress_vector_widget = pg.PlotWidget(
            title="overall_progress_vector")
        ...
        hex_color = "#{0:02x}{1:02x}{2:02x}".format(
            clamp(int(GYMY_GREEN[0])), clamp(int(GYMY_GREEN[1])), clamp(int(GYMY_GREEN[2])))
        ...
        self.overall_progress_vector_widget.addItem(
            self.overall_progress_vector_curve)
        self.overall_progress_vector_curve.setData(
            np.array([0, 1]), np.array([0, 0]), pen=GYMY_GREEN)
    ...
    def _update_overall_data(self, progress, alignment, progress_alignment_vector, score, last_score):
        """Updates data that is not directly related to a single feature, but to the sum of all features."""
        ...
        self.overall_progress_vector_curve.setData(
            self._progress_vector_x, self._progress_vector_y, pen=pen_color)
        hex_color = "#{0:02x}{1:02x}{2:02x}".format(
            clamp(int(pen_color[0])), clamp(int(pen_color[1])), clamp(int(pen_color[2])))
        style = "QProgressBar::chunk \
            {{background-color: {};}} \
            text-align: center;".format(hex_color)
        ...
        pen_color = tuple(np.array(GYMY_ORANGE) *
                          (1 - score/100) + np.array(GYMY_GREEN) * score/100)
        hex_color = "#{0:02x}{1:02x}{2:02x}".format(
            clamp(int(pen_color[0])), clamp(int(pen_color[1])), clamp(int(pen_color[2])))
        style = "QProgressBar::chunk \
            {{background-color: {};}} \
        ...
        pen_color = tuple(np.array(GYMY_ORANGE) * (1 - last_score /
                          100) + np.array(GYMY_GREEN) * last_score/100)
        hex_color = "#{0:02x}{1:02x}{2:02x}".format(
            clamp(int(pen_color[0])), clamp(int(pen_color[1])), clamp(int(pen_color[2])))
        style = "QProgressBar::chunk \
            {{background-color: {};}} \
    ...
    def trigger_freeze_graphs(self):
        """Set freeze flag for all feature widgets. We use this to analyze trajectories by hand and make them stand still for a moment."""
        for fw in self.feature_widgets.values():
            fw.frozen = not fw.frozen
        self.alignment_widget.setUpdatesEnabled(self.widgets_frozen)
        self.overall_progress_vector_widget.setUpdatesEnabled(
            self.widgets_frozen)
    ...
    def create_feature_widgets(self):
        ...
        if spot_data:
            ...
            self.setFixedWidth(
                400 * (1 + (len(self.spot_data.get(self.chosen_spot).get("feature_hashes") or 0))))
