...
class BaseFeature(ABC):
    ...
    def __init__(self,
        ...
        self.max_digitized_traj_len = np.int(np.min(
            [self.config['DIGITIZED_FEATURE_TRAJECTORY_MAX_MEMORY_SIZE'], max_digitized_trajectory_length]))
        ...
        from src.algorithm.FeatureExtraction import feature_extraction_methods
        self.feature_extraction_method = feature_extraction_methods[self.type]

    def add_filter_value(self, value: float) -> NoReturn:
        try:
            filtered_value = max(self.filtered_value - (self.config['TRUST_REGION_FILTER_FACTOR'] * self.resolution), min(
                value, self.filtered_value + (self.config['TRUST_REGION_FILTER_FACTOR'] * self.resolution)))
    ...
    def update_pose(self, pose: np.ndarray, pose_definition_adapter: PoseDefinitionAdapter) -> bool:
        """Updates the feature values of this feature according to a pose.

        Args:
            pose: A pose as an ndarray
            pose_definition_adapter: A pose definition adapter to use

        Returns:
            has_changed_progression: Is true, if this pose changes the progress of the feature
        """
        value = self.feature_extraction_method(
            pose, self.specification_dict, pose_definition_adapter)
        filtered_value = self.add_filter_value(value)

        try:
            discretized_values = discretize_feature_values(
                filtered_value, self.discretized_values[-1], self.resolution)
            if discretized_values:
                self.discretized_values = np.append(
                    self.discretized_values, discretized_values)
        except IndexError:
            scale = self.scale
        ...
        has_changed_progression = self.compute_new_feature_progression(
            self.median_beginning_state)
        self.states = np.append(self.states, decide_feature_state(
            value, self.state, self.lower_boundary, self.upper_boundary))
...
class ReferenceRecordingFeature(BaseFeature):
    ...
    def __init__(self,
                 ...
                 specification_dict: object = {}):
        super().__init__(config, feature_hash, specification_dict,
                         max_digitized_trajectory_length=np.inf)
    ...
    def update_data(self) -> NoReturn:
        ...
        This data contains information on how an expert has done a repetition, with regards to this feature.
        First add a trajectory to this feature before calling this method.
        Afterwards, call this method to translate the trajectory you added.
        """
        ...
        self.upper_boundary = self.highest_value - self.range_of_motion * \
            self.config['REDUCED_RANGE_OF_MOTION_TOLERANCE_HIGHER']

        for value in self.values:
            self.add_filter_value(value)  # We need the resolution for this

        first_reference_feature_value = self.values[0]
        self.beginning_state = decide_feature_state(
            first_reference_feature_value, FeatureState.FEATURE_UNDECIDED, self.lower_boundary, self.upper_boundary)
        ...
        self.number_of_dicided_state_changes = compute_number_of_decided_state_changes(
            median_feature_states_array)

        self.median_trajectory, self.median_trajectory_feature_states, self.median_trajectory_discretization_ranges = compute_median_discrete_trajectory_median_feature_states_and_reference_trajectory_fractions(
            discrete_trajectories_tensor, self.discretization_reference_trajectory_indices_tensor, self.lower_boundary, self.upper_boundary, [self.recording_length])
    ...
    @property
    def average_total_joint_difference_last_rep(self) -> float:
        return np.average(self.total_joint_differences_this_rep)

    def new_repetition(self) -> NoReturn:
        """Reset this reference feature's progression and joint differences"""
        self.progression = 0
        self.total_joint_differences_this_rep = []
...
class Feature(BaseFeature):
    def __init__(self,
        ...
        super().__init__(config, feature_hash, feature_specification,
                         max_digitized_trajectory_length=np.inf)
    ...
    def add_recording(self, exercise_id, recording: np.ndarray, pose_definition_adapter, config=None):
        ...
        if not config:
            config = self.config
        reference_feature = ReferenceRecordingFeature(
            config, self.feature_hash, exercise_id, recording, pose_definition_adapter, self.specification_dict)
        self.reference_recording_features.append(reference_feature)

    def update_static_data(self) -> NoReturn:
        """(Re-)Calculate the information that we find in this reference feature trajectories.
        ...
        filtered_trajectories = [
            rf.filtered_values for rf in self.reference_recording_features]
        ...
        self.median_trajectory, self.median_trajectory_feature_states, self.median_trajectory_discretization_ranges = compute_median_discrete_trajectory_median_feature_states_and_reference_trajectory_fractions(
            discrete_trajectories_tensor, self.discretization_reference_trajectory_indices_tensor, self.lower_boundary, self.upper_boundary, recording_lengths)
        # TODO: Check wether median_reference_trajectory_feature_states match the median state trajectory

        beginning_states = [
            r.beginning_state for r in self.reference_recording_features]
    ...
    def new_repetition(self):
        """Reset all reference feature's progression and joint differences for this feature."""
        self.progression = 0
        self.total_joint_differences_this_rep = []
                ...
                for j in range(0, len(trajectory) - 3 * i):
                    ...
                    if np.array_equal(window[:i], window[2 * i:]):
                        ...
                        try:
                            ...
                            trajectory = np.delete(
                                trajectory, range(j, j + 2 * i))
...
def compute_discrete_trajectories_hankel_matrices_and_feature_states(feature_trajectories, resolution, lower_boundary, upper_boundary, config):
    ...
    discrete_trajectories_tensor = []
    # For every resampled value, we need an index to point us to the original pose
    discretization_reference_trajectory_indices_tensor = []
    # In the analysis algorithm, we need a hankel matrix of every resampled reference trajectory
    hankel_tensor = []
    feature_states_matrix = list()

    for trajectory in feature_trajectories:
        last_values = [trajectory[0]]
        last_feature_state = decide_feature_state(
            last_values[-1], None, lower_boundary, upper_boundary)
        ...
        for index, value in enumerate(trajectory):
            already_discritized_values = discretize_feature_values(
                value, last_values[-1], resolution)
            if already_discritized_values:
                already_discritized_values = np.array(
                    already_discritized_values)
                discrete_values = np.append(
                    discrete_values, already_discritized_values)
                feature_state = decide_feature_state(
                    already_discritized_values[-1], last_feature_state, lower_boundary, upper_boundary)
        ...
        # The first column of the hankel matrix represents the last frame of the recording
        hankel_matrix = np.roll(hankel_matrix, -1, axis=0)
        hankel_matrix = np.array(hankel_matrix, dtype=np.float16)
        hankel_tensor.append(hankel_matrix)

        feature_trajectory_indices = np.array(
            feature_trajectory_indices, dtype=np.int16)
        discretization_reference_trajectory_indices_tensor.append(
    ...
    discretization_reference_trajectory_indices_tensor = np.asarray(
        discretization_reference_trajectory_indices_tensor, dtype=object)
...
def compute_median_feature_states(feature_states_matrix):
    ...
    for i in range(median_length):
        feature_states_at_i = [np.take(feature_states_matrix[j], i,  mode='clip')
                               for j in range(len(feature_states_matrix))]
        ...
        while not np.where(feature_states_matrix != median_feature_state):
            bad_feature_state_indices = np.where(
                feature_states_matrix != median_feature_state)
            ...
            for bad_feature_state_index in bad_feature_state_indices:
                if len(feature_states_matrix[bad_feature_state_index]) > median_length:
                    ...
                    np.insert(feature_states_matrix,
                              bad_feature_state_index, median_feature_state)
...
def compute_median_discrete_trajectory_median_feature_states_and_reference_trajectory_fractions(discrete_trajectories_tensor, discretization_reference_trajectory_indices_tensor, lower_boundary, upper_boundary, recording_lengths):
    ...
    median_length = np.int(
        np.median([len(values) for values in discrete_trajectories_tensor]))
    for i in range(median_length):
        median_resampled_values_reference_trajectory_fraction_from = np.average([np.take(
            discretization_reference_trajectory_indices_tensor[j], i,  mode='clip')/recording_lengths[j] for j in range(len(recording_lengths))])
        median_resampled_values_reference_trajectory_fraction_to = np.average([np.take(
            discretization_reference_trajectory_indices_tensor[j], i + 1,  mode='clip')/recording_lengths[j] for j in range(len(recording_lengths))])
        median_feature_value = np.median(
            [np.take(a, i, mode='clip') for a in discrete_trajectories_tensor])
        done = False
        while not done:
            values = np.array([np.take(a, i, mode='clip')
                              for a in discrete_trajectories_tensor])
            done = np.where(np.isclose(values, median_feature_value))
            bad_feature_state_indices = np.where(np.isclose(
                values, median_feature_value))  # Same as variable "done"
            # Modify trajectories that differ such that they stay useful
            for bad_feature_state_index in bad_feature_state_indices[0].tolist():
                if values[bad_feature_state_index].size > median_length:
                    ...
                    np.delete(
                        discrete_trajectories_tensor[bad_feature_state_index], i)
                    ...
                    try:
                        ...
                        discrete_trajectories_tensor[bad_feature_state_index] = np.append(
                            discrete_trajectories_tensor[bad_feature_state_index], median_feature_value)
    ...
    for index, value in enumerate(median_trajectory):
        feature_state = decide_feature_state(
            value, last_feature_state, lower_boundary, upper_boundary)
        last_feature_state = feature_state
        if value == last_value:
...
def compute_number_of_decided_state_changes(feature_states_array):
    ...
    decided_median_feature_states = feature_states_array[abs(
        feature_states_array) > 1]
    decided_median_feature_state_change_indices = np.where(
        decided_median_feature_states[:-1] * decided_median_feature_states[1:] < 0)[X] + 1
    number_of_decided_state_changes = len(
        decided_median_feature_state_change_indices)
