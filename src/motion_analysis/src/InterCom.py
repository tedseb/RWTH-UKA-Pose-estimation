...
class SpotQueueInterface(ABC):
    ...
    def dequeue(self, spot_key: str) -> Tuple[list, list, list]:
        ...
        raise NotImplementedError(
            "This is an interface and shold not be called directly")

    @abstractmethod
    def enqueue(self, spot_key: str, message: Any) -> NoReturn:
...
class MessageQueueInterface(ABC):
    ...
    @abstractmethod
    def delete(self, spot_key: str) -> NoReturn:
        raise NotImplementedError(
            "This is an interface and shold not be called directly")
...
class QueueLoadBalancerInterface(ABC):
    ...
    @abstractmethod
    def set_queue_size(self, key: str, queue_size: int) -> NoReturn:
...
class SpotMetaDataInterface(ABC):
    ...
    @abstractmethod
    def set_spot_info_dict(self, key: str, spot_info_dict: dict) -> NoReturn:
...
class FeaturesInterface(ABC):
    ...
    @abstractmethod
    def set(self, spot_key: str, featuers_dict: dict,  reset: bool = False) -> NoReturn:
...
class RedisFeaturesInterface(RedisInterface, FeaturesInterface):
    ...
    def __init__(self):
        ...
        self.redis_connection = redis.StrictRedis(
            host='localhost', port=5678, db=0, decode_responses=False)
    ...
    def set(self, spot_features_key: str, features_dict: dict) -> NoReturn:
        _features_dict = {}
    ...
    def delete(self, spot_features_key: str) -> NoReturn:
...
class RedisSpotMetaDataInterface(RedisInterface, SpotMetaDataInterface):
    def __init__(self):
        ...
        # We need a separate constructor in order to force decode_responses=True for this Redis connection
        self.redis_connection = redis.StrictRedis(
            host='localhost', port=5678, db=0, decode_responses=True)

    def get_spot_info_dict(self, key: str, info_keys: list) -> dict:
        spot_info_list = self.redis_connection.hmget(key, info_keys)
        if None in spot_info_list:
            raise NoSpotMetaDataSetExcpetion(
                "Trying to get spot_info_dict at key " + str(key) + ", but no spot_info_dict is set.")
        spot_info_dict = dict(zip(info_keys, spot_info_list))
        if "exercise_data" in spot_info_dict.keys() and spot_info_dict["exercise_data"]:
            spot_info_dict["exercise_data"] = yaml.load(
                spot_info_dict["exercise_data"], Loader=yaml.Loader)
    ...
    def set_spot_info_dict(self, key: str, spot_info_dict: dict) -> NoReturn:
        ...
        if "exercise_data" in spot_info_dict.keys():
            ...
            spot_info_dict_copy["exercise_data_hash"] = hashlib.md5(
                yaml_string.encode('utf-8')).hexdigest()
...
class RedisQueueLoadBalancerInterface(RedisInterface, QueueLoadBalancerInterface):
    def get_queue_key(self):
        ...
        try:
            key_tuple = self.redis_connection.bzpopmax(
                self.config['REDIS_LOAD_BALANCER_SORTED_SET_KEY'], timeout=1)
        except TimeoutError:
            raise QueueEmpty
...
class RedisMessageQueueInterface(RedisInterface, MessageQueueInterface):
    ...
    def dequeue(self, key: str) -> object:
        try:
            ...
            if not message:
                raise QueueEmpty
            # TODO: Python 2.7 bytestrings anywhere?
            message = msgpack.unpackb(message)
            try:
                message = message.decode('utf-8')
            except Exception:
            ...
            logy.error("Issue getting message from Queue: " +
                       str(key) + ' Error: \n' + str(e))
...
class RedisSpotQueueInterface(RedisInterface, SpotQueueInterface):
    def __init__(self, config):
        ...
        self.redis_connection = redis.StrictRedis(
            connection_pool=redis_connection_pool)
        self.ma_validation_rate_control = rp.Publisher(
            "ma_validation_rosbag_rate_control", Int32, queue_size=10)
    ...
    def dequeue(self, spot_key: str) -> Tuple[list, list, list]:
        ...
        spot_queue_key, spot_past_queue_key, _, _ = generate_redis_key_names(
            spot_key, self.config)

        try:
            joints_with_timestame_bytes = self.redis_connection.rpoplpush(
                spot_queue_key, spot_past_queue_key)
            if not joints_with_timestame_bytes:
                raise QueueEmpty
            joints_with_timestamp = msgpack.unpackb(
                joints_with_timestame_bytes)
        ...
        self.redis_connection.ltrim(
            spot_past_queue_key, 0, self.config['REDIS_MAXIMUM_PAST_QUEUE_SIZE'])

        future_joints_with_timestamp_list = self.redis_connection.lrange(
            spot_queue_key, 0, self.config['REDIS_MAXIMUM_PAST_QUEUE_SIZE'])
        past_joints_with_timestamp_list = self.redis_connection.lrange(
            spot_past_queue_key, 0, self.config['REDIS_MAXIMUM_PAST_QUEUE_SIZE'])
    ...
    def enqueue(self, spot_key: str, data: Any) -> int:
        spot_queue_key, _, _, _ = generate_redis_key_names(
            spot_key, self.config)

        queue_size = self.redis_connection.rpush(
            spot_queue_key, msgpack.packb(data))
        ...
        if (queue_size >= self.config['REDIS_QUEUE_SIZE_PANIC_BOUNDARY']):
            if (queue_size >= self.config['REDIS_MAXIMUM_QUEUE_SIZE']):
                ...
                self.redis_connection.ltrim(
                    spot_queue_key, 0, self.config['REDIS_MAXIMUM_QUEUE_SIZE'])
            else:
                pass
    ...
    def size(self, spot_key: str):
        spot_queue_key, spot_past_queue_key, spot_info_key, spot_features_key = generate_redis_key_names(
            spot_key, self.config)
...
def generate_redis_key_names(spot_key: str, config) -> List[str]:
    ...
    """
    redis_spot_queue_key = config['REDIS_GENERAL_PREFIX'] + config['REDIS_KEY_SEPARATOR'] + str(
        spot_key) + config['REDIS_KEY_SEPARATOR'] + config['REDIS_SPOT_QUEUE_POSTFIX']
    redis_spot_past_queue_key = config['REDIS_GENERAL_PREFIX'] + config['REDIS_KEY_SEPARATOR'] + \
        redis_spot_queue_key + \
        config['REDIS_KEY_SEPARATOR'] + config['REDIS_SPOT_PAST_QUEUE_POSTFIX']
    redis_spot_info_key = config['REDIS_GENERAL_PREFIX'] + config['REDIS_KEY_SEPARATOR'] + str(
        spot_key) + config['REDIS_KEY_SEPARATOR'] + config['REDIS_SPOT_INFO_POSTFIX']
    redis_spot_features_key = config['REDIS_GENERAL_PREFIX'] + config['REDIS_KEY_SEPARATOR'] + str(
        spot_key) + config['REDIS_KEY_SEPARATOR'] + config['REDIS_SPOT_FEATURES']
