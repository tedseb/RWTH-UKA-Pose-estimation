...
class Receiver():
    ...
    def callback(self, message: Any) -> NoReturn:
        """
        ...
        for p in message.persons:
            array = self.pose_definition_adapter.body_parts_to_ndarray(
                p.bodyParts)
            joints_with_timestamp = {
                'used_joint_ndarray': array, 'ros_timestamp': message.header.stamp.to_time()}
            queue_size_increment_value = self.spot_queue_interface.enqueue(
                p.stationID, joints_with_timestamp)
...
if __name__ == '__main__':
    ...
    # TODO: @sm This does not do anything now - is it even necessary?
    parser.add_argument("-v", "--verbose",
                        help="Verbose mode", action="store_true")
    parser.add_argument(
        "-a", "--ai", help="Name of AI to work with", type=str, default='metrabs')
    parser.add_argument("-c", "--configpath", help="Path to config file", type=str,
                        default='/home/trainerai/trainerai-core/src/motion_analysis/config.yml')
    ...
    if args.ai == 'spin':
        ...
        rp.logerr(
            "Could not find a suitable PoseDefinition Adapter for ai argument: <" + str(args.ai) + ">")

    with open(args.configpath, 'r') as infile:
        config = yaml.safe_load(infile)

    receiver = Receiver(
        config, pose_definition_adapter_class=pose_definition_adapter_class)
